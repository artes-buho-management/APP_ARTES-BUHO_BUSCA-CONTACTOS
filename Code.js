const APP_CONFIG = Object.freeze({
  ...
  antiLoopDomainSuppressionMinHits: 2,
  ...
  leadDiscoveryProvider: "GOOGLE_PLACES_API",
  placesStrictDiscoveryOnly: true,
  placesApiKeyPropertyKey: "GOOGLE_PLACES_API_KEY",
  placesApiKeyPropertyAliases: [
  ...
  placesRequestTimeoutMs: 25000,
  placesRequestMaxAttempts: 3,
  placesRequestRetryBaseMs: 900,
  placesRequestRetryMaxMs: 5000,
  placesMaxResultsPerRequest: 20,
  placesMaxPagesPerQuery: 2,
  placesMaxQueriesPerStep: 24,
  placesMaxAcceptedPerQuery: 5,
  placesMaxAcceptedPerQueryDiversity: 2,
  placesPerDomainCapPerStep: 1,
  placesPerDomainCapPerJob: 4,
  placesDominantDomainBlockPerStep: 2,
  placesForceDomainDiversityAfterStaleRounds: 2,
  placesForceDomainDiversityAfterIterations: 3,
  placesForceDomainDiversityFoundFloor: 18,
  placesUsePersistentSeenDomainMemory: true,
  placesSeenDomainMemoryCap: 4000,
  placesUsePersistentSeenCompanyMemory: true,
  placesSeenCompanyMemoryCap: 5000,
  placesSeenPlaceIdsMemoryCap: 12000,
  placesMarkDroppedPlaceIdsSeen: true,
  placesSkipQueryAfterNoGainCount: 2,
  placesQueryNoGainMemoryCap: 1500,
  placesPerCompanyCapPerStep: 2,
  placesPerCompanyCapPerJob: 6,
  verifyPerDomainCapPerStep: 2,
  verifyPerDomainCapPerJob: 8,
  verifyPerDomainCapRelaxAfterStaleRounds: 4,
  placesRequireWebsiteForEmailWorkflows: true,
  ...
  searchProfiles: {
    ...
    AGRESIVO_CAUDAL: {
      ...
      hybridPerDomainCapPerStep: 2,
      placesPerDomainCapPerStep: 1,
      placesPerDomainCapPerJob: 2,
      verifyPerDomainCapPerStep: 1,
      verifyPerDomainCapPerJob: 3
    }
  },
  crmTargetSheets: [
    "RELEVANTES INDIE",
...
function startSearchJob(request) {
  ...
  const existingDedupState = cfg.clearPreviousData
    ? { seenHashes: [], seenEmails: [], seenCompanies: [] }
  ...
  const placesDiscoveryEnabled = isPlacesDiscoveryEnabled_();
  if (!placesDiscoveryEnabled && !hybridConfig.enabled && !geminiCanRunAsPrimary) {
    throw new Error("No hay motor de busqueda activo. Configura Google Places API o un endpoint hibrido.");
  }
  enforceLocalOnlyRuntimeOrThrow_(hybridConfig);
  if (placesDiscoveryEnabled) {
    const incomingPlacesApiKey = String(cfg.placesApiKey || "").trim();
    if (incomingPlacesApiKey) {
      savePlacesApiKey_(incomingPlacesApiKey);
    }
    resolvePlacesApiKeyOrThrow_();
  }
  if (!placesDiscoveryEnabled && hybridConfig.enabled && APP_CONFIG.hybridHealthCheckOnStart === true) {
  ...
  const state = {
    ...
    queryExcludedDomains: Array.isArray(searchOptions.excludedDomains) ? searchOptions.excludedDomains.slice(0) : [],
    query: cfg.query,
    scope: cfg.scope,
    ...
    seenCompanies: existingDedupState.seenCompanies,
    placesSeenDomainCount: {},
    placesSeenCompanyCount: {},
    placesSeenPlaceIds: {},
  ...
  state.placesSeenDomainCount = buildRootDomainCountFromEmails_(state.seenEmails);
  state.placesSeenCompanyCount = buildCompanyCountFromNames_(state.seenCompanies);
  ...
  if (state.queryExcludedDomains && state.queryExcludedDomains.length) {
    pushLog_(state, `Exclusiones por dominio detectadas en prompt: ${state.queryExcludedDomains.join(", ")}`);
  }
  ...
  if (placesDiscoveryEnabled) {
    pushLog_(state, "Discovery activo: Google Places API (Google Maps Platform).");
    pushLog_(state, "Discovery web: NO se usa scraping de Google Search.");
    if (APP_CONFIG.placesStrictDiscoveryOnly === true) {
      pushLog_(state, "Modo estricto Places: sin fallback a motores remotos de descubrimiento.");
    }
  }
  if (!placesDiscoveryEnabled && state.hybrid && state.hybrid.enabled) {
  ...
  } else if (!placesDiscoveryEnabled && state.hybrid && state.hybrid.enabledByOverride && !state.hybrid.endpointReady && state.geminiAllowed) {
    pushLog_(state, "Modo hibrido solicitado pero sin endpoint valido. Se usa IA remota como motor principal.");
  } else if (!placesDiscoveryEnabled) {
...
function runSearchJobStep(jobId) {
  ...
  try {
    ...
    if (placesDiscoveryEnabled) {
      try {
        stepResult = callPlacesApiCandidates_(state, queryVariant, desired);
        state.hybridUnavailableStreak = 0;
        if (stepResult && Number(stepResult.sourceCandidatesCount || 0) > 0) {
          pushStepLog_(
            state,
            stepLogs,
            `Google Places API candidatos: ${stepResult.sourceCandidatesCount}. Discovery sin scraping de Google Search.`
          );
        } else {
          pushStepLog_(state, stepLogs, "Google Places API sin candidatos nuevos en este paso.");
        }
      } catch (placesErr) {
        const placesErrMsg = formatErrorForLog_(placesErr);
        pushStepLog_(state, stepLogs, `Aviso Google Places API en este paso: ${placesErrMsg}`);
      }
    } else if (hybridCfg.enabled && hybridCfg.preferCodeFirst) {
      try {
        stepResult = callHybridScraperCandidates_(state, queryVariant, desired);
        state.hybridUnavailableStreak = 0;
        if (stepResult && Number(stepResult.sourceCandidatesCount || 0) > 0) {
          state.hybridStepsUsed = Number(state.hybridStepsUsed || 0) + 1;
          state.hybridCandidatesTotal = Number(state.hybridCandidatesTotal || 0) + Number(stepResult.sourceCandidatesCount || 0);
          pushStepLog_(
            state,
            stepLogs,
            `Hibrido candidatos: ${stepResult.sourceCandidatesCount}. IA remota evitada en primera fase para ahorrar coste.`
          );
        } else {
    ...
    const mustRunGeminiByRules = shouldRunGeminiFallback_(state, stepResult, desired);
    const strictPlacesOnly = placesDiscoveryEnabled && APP_CONFIG.placesStrictDiscoveryOnly === true;
    const mustRunGemini = strictPlacesOnly ? false : mustRunGeminiByRules;
    if (strictPlacesOnly && mustRunGeminiByRules) {
      pushStepLog_(
        state,
        stepLogs,
        "Fallback remoto bloqueado: modo estricto Places activo en esta ejecucion."
      );
    }
    if (!mustRunGemini && state.geminiAllowed === false) {
      if (placesDiscoveryEnabled) {
        pushStepLog_(state, stepLogs, "IA remota desactivada por perfil: discovery activo solo con Google Places API.");
      } else {
  ...
  if (stepDiag && Number(stepResult.sourceCandidatesCount || 0) <= 0) {
    ...
    const diagDropCompanyCap = pickPositiveMetric_(
      [stepDiag.drop_per_company_cap, stepDiag.skip_company_cap],
      0
    );
    pushStepLog_(
      state,
      stepLogs,
      `Hibrido diagnostico: pages_scanned=${diagPagesScanned} | pages_irrelevant=${diagPagesIrrelevant} | pages_scope_mismatch=${diagPagesScopeMismatch} | emails_candidates=${diagEmailsCandidates} | drop_nombre=${diagDropNombre} | drop_nombre_email=${diagDropNombreEmail} | drop_empresa=${diagDropEmpresa} | drop_role_mailbox=${diagDropRoleMailbox} | drop_seen=${diagDropSeen} | drop_company_cap=${diagDropCompanyCap}.`
    );
  }
  ...
  const dedupe = dedupeRecords_(normalized, state.seenHashes, state.seenEmails, state.seenCompanies);
...
function crmDumpFromHybridV2_(params) {
  ...
  if (placesDiscoveryEnabled) {
    resolvePlacesApiKeyOrThrow_();
    const placesState = {
      jobId: `crm_${sheetName}_${Date.now()}`,
      query: query,
      scope: "NACIONAL_ES",
      profile: profile,
      profileConfig: profileConfig,
      ...
      strictRequireEmailOnWeb: false,
      strictRequirePhoneOnWeb: false,
      strictRequireContactNameOnWeb: false,
      strictRequireCompanyOnWeb: false,
      seenEmails: fastIndex.list.slice(-Math.max(100, Number(APP_CONFIG.crmExcludeEmailsWindow || 800))),
      domainStats: {},
      staleRounds: 1,
      iterations: 1,
      dynamicPlanExpansions: 0,
    ...
    const placesResult = callPlacesApiCandidates_(placesState, query, desiredPool);
    contactsRaw = Array.isArray(placesResult.contacts) ? placesResult.contacts : [];
    sourceEngine = String(placesResult.source || "google_places_api");
    sourceMeta = (placesResult.meta && typeof placesResult.meta === "object") ? placesResult.meta : {};
  } else {
    const hybridCfg = resolveHybridRuntimeConfig_(profileConfig);
    enforceLocalOnlyRuntimeOrThrow_(hybridCfg);
    if (!hybridCfg.enabled || !hybridCfg.endpointReady) {
    ...
    contactsRaw = Array.isArray(hybridRaw.rawContacts) ? hybridRaw.rawContacts : [];
    sourceEngine = String(hybridRaw.sourceEngine || "hybrid_endpoint");
    sourceMeta = (hybridRaw.parsedMeta && typeof hybridRaw.parsedMeta === "object") ? hybridRaw.parsedMeta : {};
  }
  ...
  const minSpainScore = isAggressiveCaudal ? 12 : Number(thresholds.MIN_SPAIN_SCORE || 40);
  const minScorePersonal = isAggressiveCaudal ? 18 : Number(thresholds.MIN_SCORE_PERSONAL || 45);
  const minScoreRole = isAggressiveCaudal ? 16 : Number(thresholds.MIN_SCORE_ROLE || 50);
  const minScoreWeak = isAggressiveCaudal ? 16 : Number(thresholds.MIN_SCORE_WEAK_GENERIC || 65);
  ...
  for (let i = 0; i < normalizedContacts.length; i += 1) {
    ...
    const emailDomainHint = String(candidate.emailDomain || getEmailDomain_(email) || "").trim().toLowerCase();
    const sourceDomainHint = String(candidate.sourceDomain || "").trim().toLowerCase();
    const sourceUrlHint = String(candidate.sourceUrl || "").trim().toLowerCase();
    const companyHint = String(candidate.company || "").trim().toLowerCase();
    const hasSpainHint = (
      /\.es$/i.test(emailDomainHint)
      || /\.es$/i.test(sourceDomainHint)
      || /(?:^|[\W_])(espana|españa)(?:$|[\W_])/i.test(companyHint)
      || /(?:^|[\W_])(espana|españa)(?:$|[\W_])/i.test(sourceUrlHint)
    );
    if ((spainEval.rejected || candidate.spainScore < minSpainScore) && !(isAggressiveCaudal && hasSpainHint)) {
      incrementCrmReason_(reasons, "NO_SPAIN_SIGNAL");
      continue;
    }
    if (isAggressiveCaudal && hasSpainHint && (spainEval.rejected || candidate.spainScore < minSpainScore)) {
      incrementCrmReason_(reasons, "SPAIN_RELAXED_HINT");
    }
    afterSpain += 1;
    ...
    const emailLocalPart = String((candidate.email || "").split("@")[0] || "").trim().toLowerCase();
    const isRoleLikeLocalPart = !!emailLocalPart && isRoleLikeMailboxToken_(emailLocalPart);
    if (candidate.name && !isRoleLikeLocalPart) {
      const normalizedName = normalizePersonName_(String(candidate.name || "").trim());
      const domainHint = candidate.sourceDomain || candidate.sourceUrl || "";
      const nameCoherentByEmail = normalizedName
        && (
          isNameConsistentWithEmailLocal_(normalizedName, candidate.email)
          || isSingleTokenNameAllowedByEmail_(normalizedName, candidate.email, candidate.company || "", domainHint)
        );
      if (normalizedName && !nameCoherentByEmail) {
        const repairedName = normalizePersonName_(String(
          inferNameFromEmail_(candidate.email)
          || guessSingleNameFromEmailLocal_(candidate.email)
          || guessContactNameFromEmail_(candidate.email)
          || ""
        ).trim());
        const repairedByEmail = repairedName
          && (
            isNameConsistentWithEmailLocal_(repairedName, candidate.email)
            || isSingleTokenNameAllowedByEmail_(repairedName, candidate.email, candidate.company || "", domainHint)
          );
        if (repairedByEmail) {
          candidate.name = repairedName;
          incrementCrmReason_(reasons, "NAME_FIXED_FROM_EMAIL");
        } else {
          candidate.name = "";
          incrementCrmReason_(reasons, "NAME_EMAIL_MISMATCH");
        }
      }
    }
    if (!candidate.name && isAggressiveCaudal) {
      ...
      if (inferred) {
        ...
        if (!candidate.name && candidate.company) {
          candidate.name = `Contacto de ${candidate.company}`;
          incrementCrmReason_(reasons, "NAME_FILLED_GENERIC_COMPANY");
        }
      }
    }
  ...
  return {
    ...
    source: sourceEngine || "hybrid_endpoint",
    meta: sourceMeta
  };
}
...
function initCrmReasonStats_() {
  return {
    ...
    NAME_EMAIL_MISMATCH: 0,
    NAME_FIXED_FROM_EMAIL: 0,
    APPENDED: 0
  };
}
...
function detectRoleVerticalFunctionFromEmail_(email) {
  ...
  if (/(management|manager|artistico|a&r|(?:^|\b)ar(?:\b|$))/.test(local)) return "MANAGEMENT";
...
function normalizeAllowedFunction_(raw) {
  ...
  if (/(a&r|artist and repertoire|(?:^|\b)ar(?:\b|$))/.test(value)) return "A&R";
  return "OTRO";
}
...
function doGet(e) {
  ...
  const storedToken = getDoGetAuthToken_();
  if (!storedToken) {
    return ContentService
      .createTextOutput(JSON.stringify({ ok: false, error: "doGet no configurado: falta DOGET_AUTH_TOKEN en Script Properties." }))
  ...
  } else if (action === "places_status") {
    payload = getPlacesApiConfigStatus();
...
function getDoGetAuthToken_() {
  try {
    return String(PropertiesService.getScriptProperties().getProperty("DOGET_AUTH_TOKEN") || "").trim();
  } catch (err) {
    return "";
  }
}
...
function normalizeRequest_(request) {
  ...
  return {
    ...
    placesApiKey: String(request.placesApiKey || "").trim()
  };
}
...
function getSearchProfileConfig_(profileKey) {
  ...
  return {
    ...
    hybridRequestMaxSeconds: Math.max(15, Number(selected.hybridRequestMaxSeconds || APP_CONFIG.hybridRequestMaxSeconds || 26)),
    maxStaleRounds: Math.max(4, Number(selected.maxStaleRounds || APP_CONFIG.maxStaleRounds || 8)),
    forceLargeTargetStaleFloor: selected.forceLargeTargetStaleFloor !== false,
    hybridDiversifyMaxQueryAttempts: Math.max(1, Number(selected.hybridDiversifyMaxQueryAttempts || APP_CONFIG.hybridDiversifyMaxQueryAttempts || 5)),
    hybridCandidatePoolMultiplier: Math.max(1, Number(selected.hybridCandidatePoolMultiplier || APP_CONFIG.hybridCandidatePoolMultiplier || 3)),
    hybridCandidatePoolMin: Math.max(1, Number(selected.hybridCandidatePoolMin || APP_CONFIG.hybridCandidatePoolMin || 60)),
    hybridPerDomainCapPerStep: Math.max(1, Number(selected.hybridPerDomainCapPerStep || APP_CONFIG.hybridPerDomainCapPerStep || 2)),
    placesPerDomainCapPerStep: Math.max(1, Number(selected.placesPerDomainCapPerStep || APP_CONFIG.placesPerDomainCapPerStep || 1)),
    placesPerDomainCapPerJob: Math.max(1, Number(selected.placesPerDomainCapPerJob || APP_CONFIG.placesPerDomainCapPerJob || 4)),
    verifyPerDomainCapPerStep: Math.max(1, Number(selected.verifyPerDomainCapPerStep || APP_CONFIG.verifyPerDomainCapPerStep || 2)),
    verifyPerDomainCapPerJob: Math.max(1, Number(selected.verifyPerDomainCapPerJob || APP_CONFIG.verifyPerDomainCapPerJob || 8))
  };
}
...
function getPlacesApiConfigStatus() {
  const info = resolveStoredPlacesApiKeyInfo_();
  const key = String(info.key || "").trim();
  return {
    ok: true,
    configured: !!key,
    propertyKey: String(info.primaryKeyName || "GOOGLE_PLACES_API_KEY"),
    detectedInKey: String(info.keyName || ""),
    aliasesTried: Array.isArray(info.triedKeys) ? info.triedKeys : [],
    keyPreview: key ? `${key.slice(0, 4)}***${key.slice(-3)}` : "",
    provider: String(APP_CONFIG.leadDiscoveryProvider || "")
  };
}
...
function buildSearchOptions_(query, columns, profileConfig) {
  ...
  const excludedDomains = extractExcludedDomainsFromQuery_(rawQuery);
  return {
...
function extractExcludedDomainsFromQuery_(text) {
  ...
  const seen = {};
  const add = (domainRaw) => {
    let value = String(domainRaw || "").trim().toLowerCase();
    if (!value) return;
    value = value.replace(/^[*@.\-]+/g, "").replace(/[),;.!?]+$/g, "");
    if (!value) return;
    const normalized = normalizeDomainGeneric_(value) || normalizeDomainGeneric_(`https://${value}`);
    const root = getRootDomain_(normalized || value) || String(normalized || value).replace(/^www\./i, "");
    if (!root || root.indexOf(".") < 0) return;
    if (seen[root]) return;
    seen[root] = true;
    out.push(root);
  };

  const regexSiteNeg = /(?:^|\s)-\s*site:([a-z0-9.-]+\.[a-z]{2,24})\b/ig;
  let m = null;
  while ((m = regexSiteNeg.exec(raw)) !== null) {
    add(m[1]);
  }

  const regexDomainNeg = /(?:^|\s)-\s*([a-z0-9.-]+\.[a-z]{2,24})\b/ig;
  while ((m = regexDomainNeg.exec(raw)) !== null) {
    add(m[1]);
  }

  const regexDomainDirective = /\b(?:excluir|omitir|bloquear)\s+([a-z0-9.-]+\.[a-z]{2,24})\b/ig;
  while ((m = regexDomainDirective.exec(raw)) !== null) {
    add(m[1]);
  }
...
function getQueryExcludedDomainsForStep_(state, queryVariant) {
  const st = state || {};
  const base = Array.isArray(st.queryExcludedDomains) ? st.queryExcludedDomains : [];
  const stepDomains = extractExcludedDomainsFromQuery_(queryVariant || "");
  const out = [];
  const seen = {};
  const add = (raw) => {
    const root = getRootDomain_(String(raw || "").trim()) || String(raw || "").trim().toLowerCase();
    if (!root) return;
    if (seen[root]) return;
    seen[root] = true;
    out.push(root);
  };
  for (let i = 0; i < base.length; i += 1) add(base[i]);
  for (let i = 0; i < stepDomains.length; i += 1) add(stepDomains[i]);
...
function buildRootDomainCountFromEmails_(emails) {
  const out = {};
  const list = Array.isArray(emails) ? emails : [];
  const maxKeys = Math.max(100, Number(APP_CONFIG.placesSeenDomainMemoryCap || 4000));
  for (let i = 0; i < list.length; i += 1) {
    ...
    out[root] = Number(out[root] || 0) + 1;
    if (Object.keys(out).length > maxKeys) break;
  }
  return out;
}

function buildCompanyCountFromNames_(companies) {
  const out = {};
  const list = Array.isArray(companies) ? companies : [];
  const maxKeys = Math.max(120, Number(APP_CONFIG.placesSeenCompanyMemoryCap || 5000));
  for (let i = 0; i < list.length; i += 1) {
    const company = normalizeCompanyName_(String(list[i] || "").trim());
    const key = normalizeTextForMatch_(company);
...
function savePlacesApiKey_(apiKey) {
  const key = String(apiKey || "").trim();
  const keyName = String(APP_CONFIG.placesApiKeyPropertyKey || "GOOGLE_PLACES_API_KEY").trim() || "GOOGLE_PLACES_API_KEY";
  const props = PropertiesService.getScriptProperties();
  if (!key) {
...
function geminiGroundedSearchContacts_(model, queryVariant, scope, targetCount, searchOptions) {
  if (isPlacesDiscoveryEnabled_()) {
    throw new Error("Gemini con google_search desactivado: discovery configurado solo con Google Places API.");
  }
...
function shouldRunGeminiFallback_(state, currentStepResult, desired) {
  if (isPlacesDiscoveryEnabled_()) return false;
  if (APP_CONFIG.forceDisableGeminiEverywhere === true) return false;
  const st = state || {};
  const hybridCfg = (st.hybrid && typeof st.hybrid === "object") ? st.hybrid : {};
...
function getStepResultSourceLabel_(stepResult, usedGeminiFallback) {
  ...
  if (src.indexOf("places") >= 0) return "Google Places API";
  ...
  if (usedGeminiFallback) return "IA Remota";
  return isPlacesDiscoveryEnabled_() ? "Google Places API" : "Hibrido";
}

function isPlacesDiscoveryEnabled_() {
  const provider = String(APP_CONFIG.leadDiscoveryProvider || "").trim().toUpperCase();
  return provider === "GOOGLE_PLACES_API" || provider === "PLACES_API_ONLY";
}

function getPlacesApiKeyPropertyNames_() {
  const primary = String(APP_CONFIG.placesApiKeyPropertyKey || "GOOGLE_PLACES_API_KEY").trim() || "GOOGLE_PLACES_API_KEY";
  const aliasesRaw = Array.isArray(APP_CONFIG.placesApiKeyPropertyAliases)
    ? APP_CONFIG.placesApiKeyPropertyAliases
    : [];
  const out = [];
  const seen = {};
  ...
  pushName(primary);
  for (let i = 0; i < aliasesRaw.length; i += 1) {
    pushName(aliasesRaw[i]);
  }
  return out.length ? out : [primary];
}

function resolveStoredPlacesApiKeyInfo_() {
  const names = getPlacesApiKeyPropertyNames_();
  const primary = names[0];
  const props = PropertiesService.getScriptProperties();
  for (let i = 0; i < names.length; i += 1) {
    const keyName = names[i];
    const value = String(props.getProperty(keyName) || "").trim();
    if (!value) continue;
    if (i > 0) {
      // Si existe bajo alias antiguo, lo migramos al nombre principal.
      try {
        props.setProperty(primary, value);
      } catch (migrateErr) {
        // No bloqueamos por fallo de migracion.
      }
    }
    return {
      key: value,
...
function getStoredPlacesApiKey_() {
  return resolveStoredPlacesApiKeyInfo_().key;
}

function resolvePlacesApiKeyOrThrow_() {
  const info = resolveStoredPlacesApiKeyInfo_();
  if (info.key) return info.key;
  throw new Error(`Falta ${info.primaryKeyName} en Script Properties.`);
}

function sanitizeQueryForPlaces_(rawQuery) {
  let value = String(rawQuery || "").trim();
  if (!value) return "empresas industria musical espana";
  value = value.replace(/(?:^|\s)-\s*site:[^\s]+/gi, " ");
  value = value.replace(/site:[^\s]+/gi, " ");
  value = value.replace(/https?:\/\/[^\s]+/gi, " ");
  value = value.replace(/[<>{}\[\]()"'`]/g, " ");
  value = value.replace(/\b(?:and|or|not)\b/gi, " ");
  value = value.replace(/[@#]+/g, " ");
  value = value.replace(/\b(email|correo|contacto|contactos|telefono|telefonos|whatsapp|linkedin|instagram|facebook|tiktok|youtube|google|search|scraping|site)\b/gi, " ");
  value = value.replace(/\b[a-z0-9.-]+\.[a-z]{2,24}\b/gi, " ");
...
function extractPlacesFocusTerms_(rawQuery) {
  const clean = sanitizeQueryForPlaces_(rawQuery);
  const stop = {
    de: true,
    del: true,
    la: true,
  ...
  const out = [];
  const seen = {};
  const parts = String(clean || "").split(/\s+/);
  for (let i = 0; i < parts.length; i += 1) {
    const token = normalizeTextForMatch_(parts[i]);
    if (!token) continue;
...
function buildPlacesDynamicTypeHints_(rawQuery) {
  const q = normalizeTextForMatch_(String(rawQuery || ""));
  const out = [];
  const seen = {};
  const add = (value) => {
  ...
  if (/\bsello|\brecords?|\bdiscograf/.test(q)) add("sello discografico");
  if (/\bpromotor|\bpromotora|\bconciertos?/.test(q)) add("promotora musical");
  if (/\bmanagement|\bmanager/.test(q)) add("management de artistas");
  if (/\bbooking|\bcontratacion/.test(q)) add("booking de artistas");
  if (/\bfestival|\bevento/.test(q)) add("festival de musica");
  if (/\bsala/.test(q)) add("sala de conciertos");
  if (/\bradio/.test(q)) add("radio musical");
  if (/\brevista|\bmedio|\bprensa|\bperiodista|\bredactor|\bcritico/.test(q)) add("medio musical");
  if (/\bestudio|\bgrabacion|\bproduccion/.test(q)) add("estudio de grabacion");
  if (/\bagencia|\bpr|\brrpp|\bcomunicacion/.test(q)) add("agencia de prensa musical");

  const terms = extractPlacesFocusTerms_(rawQuery);
  for (let i = 0; i < terms.length; i += 1) {
    const token = terms[i];
    if (!token || token.length < 4) continue;
    if (/^(prensa|booking|manager|management|promotora|festival|sala|radio|medio|revista|agencia)$/.test(token)) continue;
    add(`empresa musical ${token}`);
  }
  return out.slice(0, 10);
}

function getPlacesLocationBiasForScope_(scope) {
  ...
  return {
    rectangle: {
      low: { latitude: 35.8, longitude: -9.5 },
      high: { latitude: 43.9, longitude: 3.5 }
    }
  };
}

function rotatePlacesQueriesForStep_(queries, seed) {
  const list = Array.isArray(queries) ? queries.filter((q) => String(q || "").trim()) : [];
  if (list.length <= 1) return list;
  const offset = Math.max(0, Number(seed || 0)) % list.length;
  if (offset === 0) return list;
  return list.slice(offset).concat(list.slice(0, offset));
}

function buildPlacesSearchQueries_(state, queryVariant) {
  const st = state || {};
  const out = [];
  const seen = {};
  const add = (raw) => {
    const value = sanitizeQueryForPlaces_(raw);
  ...
  const configuredQueries = Math.max(
    1,
    Number(
      profileCfg.placesMaxQueriesPerStep
      || APP_CONFIG.placesMaxQueriesPerStep
      || profileCfg.hybridDiversifyMaxQueryAttempts
      || APP_CONFIG.hybridDiversifyMaxQueryAttempts
      || 12
    )
  );
  const base = sanitizeQueryForPlaces_(queryVariant || st.query || "contactos industria musical espana");
  const scopeKey = normalizeScope_(st.scope || "NACIONAL_ES");
  const isMadridScope = (scopeKey === "MUNI_MADRID" || scopeKey === "PROV_COM_MADRID");
  const scopeSuffix = isMadridScope ? "madrid espana" : "espana";
  const staleRoundsNow = Math.max(0, Number(st.staleRounds || 0));
  const dynamicExpansionsNow = Math.max(0, Number(st.dynamicPlanExpansions || 0));
  const staleBonusQueries = Math.min(18, staleRoundsNow * 2 + dynamicExpansionsNow * 4);
  const maxQueries = Math.max(12, Math.min(60, configuredQueries + staleBonusQueries));

  const cityPoolNational = [
    "madrid", "barcelona", "valencia", "sevilla", "bilbao", "malaga", "zaragoza", "murcia", "vigo",
    "granada", "alicante", "valladolid", "oviedo", "santander", "pamplona", "a coruna", "las palmas",
    "salamanca", "donostia", "cordoba", "cadiz", "toledo", "almeria", "tarragona", "castellon",
    "huelva", "leon", "jaen", "burgos", "ourense", "albacete", "badajoz", "caceres", "logrono",
    "girona", "lleida", "huesca", "teruel", "cuenca", "guadalajara", "avila", "segovia", "soria",
    "palencia", "zamora", "pontevedra", "lugo", "ibiza", "mahon"
  ];
  const cityPoolMadrid = [
    "madrid", "alcobendas", "alcorcon", "fuenlabrada", "mostoles", "leganes", "getafe", "parla",
    "pozuelo", "majadahonda", "torrejon de ardoz", "las rozas"
  ];
  const cityPool = isMadridScope ? cityPoolMadrid : cityPoolNational;
  ...
  const typePoolBase = [
    ...
    "radio musical",
    "revista musical",
    "agencia de prensa musical",
    "estudio de grabacion",
    "productora de eventos musicales"
  ];
  const dynamicTypeHints = buildPlacesDynamicTypeHints_(queryVariant || st.query || "");
  const typePool = dynamicTypeHints.concat(typePoolBase);
  const focusTerms = extractPlacesFocusTerms_(queryVariant || st.query || "");
  const intentPool = [
    "contacto",
    "email",
    "web oficial",
  ...
  const idx = Math.max(
    0,
    Number(st.iterations || 0) + Number(st.dynamicPlanExpansions || 0) * 11 + Number(st.staleRounds || 0) * 5 + Number(st.found || 0)
  );
  const cityA = cityPool[idx % cityPool.length];
  const cityB = cityPool[(idx + 3) % cityPool.length];
  const cityC = cityPool[(idx + 7) % cityPool.length];
  const typeA = typePool[idx % typePool.length];
  const typeB = typePool[(idx + 2) % typePool.length];
  const typeC = typePool[(idx + 5) % typePool.length];
  const intentA = intentPool[idx % intentPool.length];
  const intentB = intentPool[(idx + 2) % intentPool.length];
  const intentC = intentPool[(idx + 4) % intentPool.length];

  const baseWordCount = base.split(/\s+/).filter((x) => x).length;
  const useBaseDirect = baseWordCount > 0 && baseWordCount <= 12;
  if (useBaseDirect) {
    add(`${base} ${scopeSuffix}`);
    add(`${base} ${cityA} ${scopeSuffix}`);
  } else {
    add(`industria musical ${scopeSuffix}`);
    add(`empresas industria musical ${scopeSuffix}`);
    add(`contactos profesionales industria musical ${scopeSuffix}`);
  }
  add(`empresas musica ${cityA} ${scopeSuffix}`);
  add(`empresas musica ${cityB} ${scopeSuffix}`);
  for (let ft = 0; ft < focusTerms.length && out.length < maxQueries; ft += 1) {
    const term = focusTerms[ft];
    if (!term) continue;
    add(`empresa musical ${term} ${scopeSuffix}`);
    if (out.length >= maxQueries) break;
    add(`empresa musical ${term} ${cityA} ${scopeSuffix}`);
  }

  add(`${typeA} ${cityA} ${intentA}`);
  add(`${typeB} ${cityB} ${intentB}`);
  add(`${typeC} ${cityC} ${intentC}`);
  add(`industria musical ${typeA} ${scopeSuffix}`);
  add(`industria musical ${typeB} ${scopeSuffix}`);
  add(`industria musical ${typeC} ${scopeSuffix}`);
  add(`agencia musical ${cityA} ${scopeSuffix} contacto`);
  add(`promotora conciertos ${cityB} ${scopeSuffix} contacto`);
  add(`sala conciertos ${cityC} ${scopeSuffix} email`);

  // Diversificacion extra: rota ciudades/tipos/intencion para abrir dominios nuevos.
  for (let i = 0; i < cityPool.length && out.length < maxQueries; i += 1) {
    const city = cityPool[(idx + i * 2) % cityPool.length];
    const type = typePool[(idx + i) % typePool.length];
    const intent = intentPool[(idx + i * 3) % intentPool.length];
    add(`${type} ${city} ${intent}`);
    if (out.length >= maxQueries) break;
    add(`empresa ${type} ${city} ${scopeSuffix}`);
  }

  const sliced = out.slice(0, maxQueries);
  return rotatePlacesQueriesForStep_(sliced, idx);
}

function requestPlacesTextSearchWithRetry_(apiKey, payloadObj) {
  const endpointBase = String(APP_CONFIG.placesApiBase || "https://places.googleapis.com/v1").replace(/\/+$/g, "");
  const endpoint = `${endpointBase}/places:searchText`;
  const attempts = Math.max(1, Number(APP_CONFIG.placesRequestMaxAttempts || 3));
  const baseWait = Math.max(250, Number(APP_CONFIG.placesRequestRetryBaseMs || 900));
  const maxWait = Math.max(baseWait, Number(APP_CONFIG.placesRequestRetryMaxMs || 5000));
    ...
    try {
      const resp = UrlFetchApp.fetch(endpoint, {
        ...
        headers: {
          ...
          "X-Goog-FieldMask": "places.id,places.displayName,places.formattedAddress,places.businessStatus,places.nationalPhoneNumber,places.internationalPhoneNumber,places.websiteUri,places.googleMapsUri,places.primaryType,places.types,nextPageToken"
        },
        payload: payload,
        escaping: false
      });
      ...
      if (code >= 200 && code < 300) {
        return parseJsonSafely_(bodyText);
      }
      lastError = `HTTP ${code}: ${truncateForLog_(bodyText, 180)}`;
      if (attempt < attempts && (code === 408 || code === 429 || code === 500 || code === 502 || code === 503 || code === 504)) {
        Utilities.sleep(Math.min(maxWait, baseWait * Math.pow(2, attempt - 1)));
        continue;
      }
      throw new Error(`Places API fallo (${lastError})`);
    } catch (err) {
      ...
      throw new Error(`Places API no disponible tras ${attempt} intento(s): ${lastError}`);
    }
  }

  throw new Error(`Places API sin respuesta valida (${lastError})`);
}

function mapPlaceToHybridCandidate_(placeObj) {
  const place = (placeObj && typeof placeObj === "object") ? placeObj : {};
  const displayName = String(
    (place.displayName && (place.displayName.text || place.displayName.displayName || "")) || place.name || ""
  ).trim();
  const website = normalizeWeb_(String(place.websiteUri || "").trim());
  const mapsUrl = normalizeWeb_(String(place.googleMapsUri || "").trim());
  const sourceUrl = website || mapsUrl;
  if (!displayName || !sourceUrl) return null;
  ...
  const domain = normalizeDomainGeneric_(website || sourceUrl);
  const placeId = String(place.id || "").trim();
  const sourceSnippet = [address, primaryType].filter((x) => x).join(" | ");
  const notes = placeId ? `place_id=${placeId}` : "";

  return {
    empresa: normalizeCompanyName_(displayName) || displayName,
    nombre_contacto: "",
    contacto: "",
    descripcion_medio_empresa: "",
    cargo: "",
...
function callPlacesApiCandidates_(state, queryVariant, targetCount) {
  const st = state || {};
  const apiKey = resolvePlacesApiKeyOrThrow_();
  const desired = Math.max(1, Number(targetCount || 1));
  const profileCfg = (st.profileConfig && typeof st.profileConfig === "object") ? st.profileConfig : {};
  const poolMultiplier = Math.max(
    1,
    Number(profileCfg.hybridCandidatePoolMultiplier || APP_CONFIG.hybridCandidatePoolMultiplier || 3)
  );
  const poolMin = Math.max(desired, Number(profileCfg.hybridCandidatePoolMin || APP_CONFIG.hybridCandidatePoolMin || desired));
  const poolTarget = Math.max(desired, Math.min(400, Math.max(poolMin, desired * poolMultiplier)));
  const rawPerDomainCap = Math.max(1, Number(profileCfg.hybridPerDomainCapPerStep || APP_CONFIG.hybridPerDomainCapPerStep || 2));
  const placesPerDomainCapStepRaw = Math.max(
    1,
    Number(profileCfg.placesPerDomainCapPerStep || APP_CONFIG.placesPerDomainCapPerStep || rawPerDomainCap || 2)
  );
  const basePerDomainCap = Math.max(1, Math.min(placesPerDomainCapStepRaw, 6));
  const basePerDomainCapPerJob = Math.max(
    basePerDomainCap,
    Number(profileCfg.placesPerDomainCapPerJob || APP_CONFIG.placesPerDomainCapPerJob || 4)
  );
  const staleRoundsNow = Math.max(0, Number(st.staleRounds || 0));
  const dynamicExpansionsNow = Math.max(0, Number(st.dynamicPlanExpansions || 0));
  const iterationsNow = Math.max(0, Number(st.iterations || 0));
  const foundNow = Math.max(0, Number(st.found || 0));
  const diversityAfterStale = Math.max(0, Number(APP_CONFIG.placesForceDomainDiversityAfterStaleRounds || 2));
  const diversityAfterIterations = Math.max(0, Number(APP_CONFIG.placesForceDomainDiversityAfterIterations || 3));
  const diversityFoundFloor = Math.max(1, Number(APP_CONFIG.placesForceDomainDiversityFoundFloor || 18));
  const forceDomainDiversity =
    (staleRoundsNow >= diversityAfterStale || dynamicExpansionsNow >= 1 || iterationsNow >= diversityAfterIterations)
    && foundNow < Math.max(diversityFoundFloor, desired * 4);
  const perDomainCap = forceDomainDiversity ? 1 : basePerDomainCap;
  const perDomainCapPerJob = forceDomainDiversity
    ? Math.max(perDomainCap, Math.min(basePerDomainCapPerJob, 2))
    : basePerDomainCapPerJob;
  const basePerCompanyCapStep = Math.max(
    1,
    Number(profileCfg.placesPerCompanyCapPerStep || APP_CONFIG.placesPerCompanyCapPerStep || 2)
  );
  const basePerCompanyCapPerJob = Math.max(
    basePerCompanyCapStep,
    Number(profileCfg.placesPerCompanyCapPerJob || APP_CONFIG.placesPerCompanyCapPerJob || 6)
  );
  const perCompanyCap = forceDomainDiversity ? 1 : basePerCompanyCapStep;
  const perCompanyCapPerJob = forceDomainDiversity
    ? Math.max(perCompanyCap, Math.min(basePerCompanyCapPerJob, 2))
    : basePerCompanyCapPerJob;
  const queries = buildPlacesSearchQueries_(st, queryVariant);
  const maxPagesPerQuery = Math.max(1, Number(APP_CONFIG.placesMaxPagesPerQuery || 2));
  const maxPerRequest = Math.max(1, Math.min(20, Number(APP_CONFIG.placesMaxResultsPerRequest || 20)));
  const maxAcceptedPerQueryBase = Math.max(
    1,
    Number(profileCfg.placesMaxAcceptedPerQuery || APP_CONFIG.placesMaxAcceptedPerQuery || 5)
  );
  const maxAcceptedPerQueryDiversity = Math.max(
    1,
    Number(profileCfg.placesMaxAcceptedPerQueryDiversity || APP_CONFIG.placesMaxAcceptedPerQueryDiversity || 2)
  );
  const maxAcceptedPerQuery = forceDomainDiversity
    ? Math.max(1, Math.min(maxAcceptedPerQueryBase, maxAcceptedPerQueryDiversity))
    : maxAcceptedPerQueryBase;
  const locationBias = getPlacesLocationBiasForScope_(st.scope || "NACIONAL_ES");
  const queryExcludedDomains = getQueryExcludedDomainsForStep_(st, queryVariant);
  const suppressedDomains = getSuppressedDomainsForStale_(st);
  let blockedDomains = buildHybridDomainExclusions_(st, suppressedDomains).concat(queryExcludedDomains);
  if (blockedDomains.length) {
    const uniqueBlocked = {};
    const merged = [];
    for (let bi = 0; bi < blockedDomains.length; bi += 1) {
      const root = String(blockedDomains[bi] || "").trim().toLowerCase();
      if (!root || uniqueBlocked[root]) continue;
      uniqueBlocked[root] = true;
      merged.push(root);
    }
    blockedDomains = merged;
  }
  if (forceDomainDiversity) {
    const queryExcludedLookup = {};
    for (let ei = 0; ei < queryExcludedDomains.length; ei += 1) {
      const root = String(queryExcludedDomains[ei] || "").trim().toLowerCase();
      if (root) queryExcludedLookup[root] = true;
    }
    const seenEmailsForDiversity = Array.isArray(st.seenEmails) ? st.seenEmails : [];
    const seenRootsForDiversity = {};
    for (let si = 0; si < seenEmailsForDiversity.length; si += 1) {
      const mail = String(seenEmailsForDiversity[si] || "").trim().toLowerCase();
      if (!isValidEmail_(mail)) continue;
      const root = getRootDomain_(String(mail.split("@")[1] || "").trim().toLowerCase());
      if (!root) continue;
      seenRootsForDiversity[root] = Number(seenRootsForDiversity[root] || 0) + 1;
    }
    const dominantRoots = Object.keys(seenRootsForDiversity)
      .filter((root) => Number(seenRootsForDiversity[root] || 0) >= 3)
      .sort((a, b) => Number(seenRootsForDiversity[b] || 0) - Number(seenRootsForDiversity[a] || 0));
    const maxDominantBlocks = Math.max(1, Math.min(4, Number(APP_CONFIG.placesDominantDomainBlockPerStep || 2)));
    for (let di = 0; di < dominantRoots.length && di < maxDominantBlocks; di += 1) {
      const root = String(dominantRoots[di] || "").trim().toLowerCase();
      if (!root || queryExcludedLookup[root]) continue;
      if (blockedDomains.indexOf(root) >= 0) continue;
      blockedDomains.push(root);
    }
  }

  let relaxedDomainSuppression = false;
  if (blockedDomains.length && shouldRelaxDomainSuppressionForState_(st) && !forceDomainDiversity) {
    blockedDomains = queryExcludedDomains.slice(0);
    relaxedDomainSuppression = true;
  }
  const blockedDomainSet = {};
  const queryExcludedDomainSet = {};
  ...
  for (let i = 0; i < queryExcludedDomains.length; i += 1) {
    const root = String(queryExcludedDomains[i] || "").trim().toLowerCase();
    if (root) queryExcludedDomainSet[root] = true;
  }

  const requireWebsiteForEmailWorkflow =
    APP_CONFIG.placesRequireWebsiteForEmailWorkflows === true
    && !!(st.strictRequireEmailOnWeb === true || (st.searchOptions && st.searchOptions.requireEmail === true));

  const contacts = [];
  const seenKeys = {};
  const perDomainCount = {};
  const perCompanyCount = {};
  const seenEmailDomainCount = {};
  const usePersistentSeenDomainMemory = APP_CONFIG.placesUsePersistentSeenDomainMemory === true;
  const usePersistentSeenCompanyMemory = APP_CONFIG.placesUsePersistentSeenCompanyMemory === true;
  const placesSeenDomainCount = (
    usePersistentSeenDomainMemory
      ? ((st.placesSeenDomainCount && typeof st.placesSeenDomainCount === "object") ? st.placesSeenDomainCount : {})
      : {}
  );
  const placesSeenCompanyCount = (
    usePersistentSeenCompanyMemory
      ? ((st.placesSeenCompanyCount && typeof st.placesSeenCompanyCount === "object") ? st.placesSeenCompanyCount : {})
      : {}
  );
  if (usePersistentSeenDomainMemory && (!st.placesSeenDomainCount || typeof st.placesSeenDomainCount !== "object")) {
    st.placesSeenDomainCount = placesSeenDomainCount;
  }
  if (usePersistentSeenCompanyMemory && (!st.placesSeenCompanyCount || typeof st.placesSeenCompanyCount !== "object")) {
    st.placesSeenCompanyCount = placesSeenCompanyCount;
  }
  if (
    usePersistentSeenCompanyMemory
    && Object.keys(placesSeenCompanyCount).length === 0
    && Array.isArray(st.seenCompanies)
    && st.seenCompanies.length
  ) {
    const seededCompanyCount = buildCompanyCountFromNames_(st.seenCompanies);
    const companyKeys = Object.keys(seededCompanyCount);
    for (let ci = 0; ci < companyKeys.length; ci += 1) {
      const key = companyKeys[ci];
      placesSeenCompanyCount[key] = Number(seededCompanyCount[key] || 0);
    }
  }
  const placesSeenPlaceIds = (st.placesSeenPlaceIds && typeof st.placesSeenPlaceIds === "object")
    ? st.placesSeenPlaceIds
    : {};
  if (!st.placesSeenPlaceIds || typeof st.placesSeenPlaceIds !== "object") {
    st.placesSeenPlaceIds = placesSeenPlaceIds;
  }
  const sourceUrlHints = [];
  const sourceUrlSeen = {};
  let totalQueriesUsed = 0;
  let totalSourceLinks = 0;
  let totalPagesFetched = 0;
  let totalExpandedPagesFetched = 0;
  let dropDomainLoop = 0;
  let dropSeenPlace = 0;
  let dropNoWebsite = 0;
  let dropPerDomainCap = 0;
  let dropPerCompanyCap = 0;
  let dropNoSource = 0;
  let dropByPromptExcludedDomain = 0;
  let dropByNoGainQuerySkip = 0;
  const markDroppedPlaceIdsSeen = APP_CONFIG.placesMarkDroppedPlaceIdsSeen === true;
  const baseNoGainThreshold = Math.max(1, Number(APP_CONFIG.placesSkipQueryAfterNoGainCount || 2));
  const noGainThreshold = forceDomainDiversity ? 1 : baseNoGainThreshold;
  const queryNoGainCount = (st.placesQueryNoGainCount && typeof st.placesQueryNoGainCount === "object")
    ? st.placesQueryNoGainCount
    : {};
  if (!st.placesQueryNoGainCount || typeof st.placesQueryNoGainCount !== "object") {
    st.placesQueryNoGainCount = queryNoGainCount;
  }

  const pushSourceHint = (urlValue) => {
    const value = normalizeWeb_(String(urlValue || "").trim());
    if (!value || sourceUrlSeen[value]) return;
    sourceUrlSeen[value] = true;
    sourceUrlHints.push(value);
  };
  const getCandidateCompanyKey = (candidate) => {
    const company = normalizeCompanyName_(String((candidate && candidate.empresa) || "").trim());
    const key = normalizeTextForMatch_(company);
  ...
  for (let i = 0; i < seenEmails.length; i += 1) {
    const email = String(seenEmails[i] || "").trim().toLowerCase();
    if (!isValidEmail_(email)) continue;
    const domain = getRootDomain_(String(email.split("@")[1] || "").trim().toLowerCase());
    if (!domain) continue;
    seenEmailDomainCount[domain] = Number(seenEmailDomainCount[domain] || 0) + 1;
  }

  const trimMemoryMap = (mapObj, maxSize) => {
    const map = (mapObj && typeof mapObj === "object") ? mapObj : null;
    const limit = Math.max(100, Number(maxSize || 1000));
    if (!map) return;
  ...
  for (let qi = 0; qi < queries.length; qi += 1) {
    ...
    const activeQuery = String(queries[qi] || "").trim();
    if (!activeQuery) continue;
    const activeQueryKey = sanitizeQueryForPlaces_(activeQuery).toLowerCase();
    if (activeQueryKey && Number(queryNoGainCount[activeQueryKey] || 0) >= noGainThreshold) {
      dropByNoGainQuerySkip += 1;
      continue;
    }
    const contactsBeforeQuery = contacts.length;
    let acceptedInQuery = 0;

    let pageToken = "";
    for (let page = 0; page < maxPagesPerQuery; page += 1) {
      if (contacts.length >= poolTarget) break;
      if (acceptedInQuery >= maxAcceptedPerQuery) break;
      const payload = {
        textQuery: activeQuery,
        languageCode: "es",
        regionCode: "ES",
        maxResultCount: maxPerRequest,
        locationBias: locationBias
      };
      if (pageToken) payload.pageToken = pageToken;

      const parsed = requestPlacesTextSearchWithRetry_(apiKey, payload);
      totalQueriesUsed += 1;
      totalPagesFetched += 1;
      if (page > 0) totalExpandedPagesFetched += 1;

      const places = Array.isArray(parsed.places) ? parsed.places : [];
      totalSourceLinks += places.length;
      for (let i = 0; i < places.length; i += 1) {
        if (contacts.length >= poolTarget) break;
        const rawPlace = (places[i] && typeof places[i] === "object") ? places[i] : {};
        const rawPlaceId = String(rawPlace.id || "").trim();
        const normalized = mapPlaceToHybridCandidate_(rawPlace);
        if (!normalized) {
          dropNoSource += 1;
          if (markDroppedPlaceIdsSeen && rawPlaceId) {
            placesSeenPlaceIds[rawPlaceId] = true;
          }
          continue;
        }
        const placeId = String(normalized.place_id || rawPlaceId || "").trim();
        if (requireWebsiteForEmailWorkflow && !String(normalized.web || "").trim()) {
          dropNoWebsite += 1;
          if (markDroppedPlaceIdsSeen && placeId) {
            placesSeenPlaceIds[placeId] = true;
          }
          continue;
        }
        if (placeId && placesSeenPlaceIds[placeId]) {
          dropSeenPlace += 1;
          continue;
        }

        const rootDomain = getRootDomain_(String(normalized.web || normalized.source_url || "").trim());
        const companyKey = getCandidateCompanyKey(normalized);
        if (rootDomain && blockedDomainSet[rootDomain]) {
          if (queryExcludedDomainSet[rootDomain]) {
            dropByPromptExcludedDomain += 1;
          }
          dropDomainLoop += 1;
          if (markDroppedPlaceIdsSeen && placeId) {
            placesSeenPlaceIds[placeId] = true;
        ...
        if (rootDomain && usePersistentSeenDomainMemory && Number(placesSeenDomainCount[rootDomain] || 0) >= perDomainCapPerJob) {
          dropDomainLoop += 1;
          if (markDroppedPlaceIdsSeen && placeId) {
            placesSeenPlaceIds[placeId] = true;
          }
        ...
        if (rootDomain && Number(seenEmailDomainCount[rootDomain] || 0) >= perDomainCapPerJob) {
          dropDomainLoop += 1;
          if (markDroppedPlaceIdsSeen && placeId) {
            placesSeenPlaceIds[placeId] = true;
          }
        ...
        if (companyKey && usePersistentSeenCompanyMemory && Number(placesSeenCompanyCount[companyKey] || 0) >= perCompanyCapPerJob) {
          dropPerCompanyCap += 1;
          if (markDroppedPlaceIdsSeen && placeId) {
            placesSeenPlaceIds[placeId] = true;
          }
        ...
        if (companyKey) {
          const usedCompany = Number(perCompanyCount[companyKey] || 0);
          if (usedCompany >= perCompanyCap) {
            dropPerCompanyCap += 1;
            if (markDroppedPlaceIdsSeen && placeId) {
              placesSeenPlaceIds[placeId] = true;
            }
        ...
        const key = [placeId, normalized.web || normalized.source_url || "", normalized.empresa || ""].join("|").trim();
        if (!key || seenKeys[key]) continue;
        seenKeys[key] = true;
        contacts.push(normalized);
        pushSourceHint(normalized.source_url || normalized.web || "");
        if (placeId) {
          placesSeenPlaceIds[placeId] = true;
        }
        if (rootDomain) {
          perDomainCount[rootDomain] = Number(perDomainCount[rootDomain] || 0) + 1;
          seenEmailDomainCount[rootDomain] = Number(seenEmailDomainCount[rootDomain] || 0) + 1;
          if (usePersistentSeenDomainMemory) {
            placesSeenDomainCount[rootDomain] = Number(placesSeenDomainCount[rootDomain] || 0) + 1;
          }
        }
        if (companyKey) {
          perCompanyCount[companyKey] = Number(perCompanyCount[companyKey] || 0) + 1;
          if (usePersistentSeenCompanyMemory) {
            placesSeenCompanyCount[companyKey] = Number(placesSeenCompanyCount[companyKey] || 0) + 1;
          }
        }
        acceptedInQuery += 1;
        if (acceptedInQuery >= maxAcceptedPerQuery) break;
    ...
    const addedInQuery = contacts.length - contactsBeforeQuery;
    if (activeQueryKey) {
      if (addedInQuery > 0) {
        queryNoGainCount[activeQueryKey] = 0;
      } else {
        queryNoGainCount[activeQueryKey] = Number(queryNoGainCount[activeQueryKey] || 0) + 1;
      }
    }
  }

  trimMemoryMap(placesSeenPlaceIds, Number(APP_CONFIG.placesSeenPlaceIdsMemoryCap || 12000));
  if (usePersistentSeenDomainMemory) {
    trimMemoryMap(placesSeenDomainCount, Number(APP_CONFIG.placesSeenDomainMemoryCap || 4000));
  }
  if (usePersistentSeenCompanyMemory) {
    trimMemoryMap(placesSeenCompanyCount, Number(APP_CONFIG.placesSeenCompanyMemoryCap || 5000));
  }
  trimMemoryMap(queryNoGainCount, Number(APP_CONFIG.placesQueryNoGainMemoryCap || 1500));

  const diagnostics = {
    pages_scanned: totalPagesFetched,
    pages_irrelevant: 0,
    pages_scope_mismatch: 0,
    emails_candidates: 0,
    drop_nombre: 0,
    drop_nombre_email: 0,
    drop_empresa: 0,
    drop_bucle_dom: dropDomainLoop,
    drop_seen_place: dropSeenPlace,
    drop_no_web: dropNoWebsite,
    drop_per_domain_cap: dropPerDomainCap,
    drop_per_company_cap: dropPerCompanyCap,
    drop_no_source: dropNoSource,
    drop_prompt_domain_excluded: dropByPromptExcludedDomain,
    drop_query_no_gain_skip: dropByNoGainQuerySkip,
    accepted_per_query_cap: maxAcceptedPerQuery
  };
  if (forceDomainDiversity) {
    diagnostics.domain_diversity_forced = 1;
    diagnostics.domain_cap_step = perDomainCap;
    diagnostics.domain_cap_job = perDomainCapPerJob;
    diagnostics.company_cap_step = perCompanyCap;
    diagnostics.company_cap_job = perCompanyCapPerJob;
  }
  if (relaxedDomainSuppression) {
    diagnostics.domain_suppression_relaxed = 1;
  }

  return {
    contacts: contacts,
    sourceCandidatesCount: contacts.length,
    webSearchQueriesCount: Math.max(0, totalQueriesUsed),
    source: "google_places_api",
    meta: {
      queriesUsed: Math.max(0, totalQueriesUsed),
      sourceLinks: Math.max(0, totalSourceLinks),
      sourceUrlsSample: sourceUrlHints.slice(0, 30),
...
function buildHybridDomainExclusions_(state, baseSuppressedDomains) {
  const st = state || {};
  const seenEmails = Array.isArray(st.seenEmails) ? st.seenEmails : [];
  const queryExcludedDomains = Array.isArray(st.queryExcludedDomains) ? st.queryExcludedDomains : [];
  const domainCount = {};
  const outMap = {};
...
function callHybridScraperCandidates_(state, queryVariant, targetCount) {
  if (isPlacesDiscoveryEnabled_()) {
    return callPlacesApiCandidates_(state, queryVariant, targetCount);
  }
  const st = state || {};
  const hybridCfg = (st.hybrid && typeof st.hybrid === "object") ? st.hybrid : {};
...
function verifyContactsWithWebSource_(contacts, state, queryVariant, desired) {
  ...
  for (let ei = 0; ei < priorEmails.length; ei += 1) {
    ...
    const root = getRootDomain_(String(email.split("@")[1] || "").trim());
    if (root) {
      verifiedDomainJobCount[root] = Number(verifiedDomainJobCount[root] || 0) + 1;
    }
  }
  ...
  const baseVerifyPerDomainCapPerStep = Math.max(
    1,
    Number(profileCfg.verifyPerDomainCapPerStep || APP_CONFIG.verifyPerDomainCapPerStep || 2)
  );
  const baseVerifyPerDomainCapPerJob = Math.max(
    baseVerifyPerDomainCapPerStep,
    Number(profileCfg.verifyPerDomainCapPerJob || APP_CONFIG.verifyPerDomainCapPerJob || 8)
  );
  const verifyPerDomainRelaxAfterStaleRounds = Math.max(
    0,
    Number(APP_CONFIG.verifyPerDomainCapRelaxAfterStaleRounds || 4)
  );
  const verifyPerDomainRelaxFactor = staleRoundsNow >= verifyPerDomainRelaxAfterStaleRounds ? 2 : 1;
  const verifyPerDomainCapPerStep = Math.max(
    1,
    Math.min(12, baseVerifyPerDomainCapPerStep * verifyPerDomainRelaxFactor)
  );
  const verifyPerDomainCapPerJob = Math.max(
    verifyPerDomainCapPerStep,
    Math.min(80, baseVerifyPerDomainCapPerJob * verifyPerDomainRelaxFactor)
  );
  ...
  for (let i = 0; i < maxToProcess; i += 1) {
    ...
    if (
      sourceRootDomain
      && Number(verifiedDomainJobCount[sourceRootDomain] || 0) >= verifyPerDomainCapPerJob
    ) {
      droppedUnverified += 1;
      droppedDomainCap += 1;
      addDiagnostic(`Descartado "${recLabel}": tope por dominio en ejecucion (${sourceRootDomain}, max=${verifyPerDomainCapPerJob}).`);
      continue;
    }
    if (
      sourceRootDomain
      && Number(verifiedDomainStepCount[sourceRootDomain] || 0) >= verifyPerDomainCapPerStep
    ) {
      droppedUnverified += 1;
      droppedDomainCap += 1;
      addDiagnostic(`Descartado "${recLabel}": tope por dominio en paso (${sourceRootDomain}, max=${verifyPerDomainCapPerStep}).`);
      continue;
    }
    ...
    const collectEvidenceFromPages = () => {
      ...
      return {
        ...
        candidateEmailVerified: candidateEmailVerified,
    ...
    let evidenceEmail = evidenceBundle.evidenceEmail;
    ...
    if (
      evidenceName
      && evidenceEmail
      && Array.isArray(detectedEmails)
      && detectedEmails.length > 1
    ) {
      const recForEmailAlignment = Object.assign({}, rec, {
        nombre_contacto: evidenceName,
        contacto: evidenceName,
        empresa: evidenceCompany || rec.empresa || ""
      });
      const alignedEmail = pickBestEmailEvidence_(
        evidenceBundle.candidateEmailVerified || evidenceEmail,
        detectedEmails,
        disallowGenericMailboxEmails,
        recForEmailAlignment,
        sourceUrl,
        pages,
        allowRoleMailboxEmails
      );
      if (alignedEmail && alignedEmail !== evidenceEmail) {
        evidenceEmail = alignedEmail;
        addDiagnostic(`Ajustado "${recLabel}": email alineado por coherencia con nombre (${evidenceEmail}).`);
      }
    }
    let emailAudit = auditEmailEvidenceStrict_(evidenceEmail, rec, pages, sourceUrl, state);
    if (
      evidenceName
      && evidenceEmail
      ...
      const repairedByEmail = normalizePersonName_(
        String(
          inferNameFromEmail_(evidenceEmail)
      ...
      if (repairedAllowed) {
        ...
        const fallbackRole = buildRoleBasedContactFallbackLabel_(
          evidenceEmail,
          evidenceCompany || rec.empresa || "",
          sourceUrl
        );
        if (fallbackRole) {
          evidenceName = fallbackRole;
          addDiagnostic(`Ajustado "${recLabel}": nombre incoherente reemplazado por etiqueta de rol (${evidenceName}).`);
        } else {
          evidenceName = "";
          addDiagnostic(`Ajustado "${recLabel}": nombre incoherente eliminado para evitar dato erroneo.`);
        }
      }
    }
    ...
    if (
      ...
      const repairedPersonName = normalizePersonName_(
        inferNameFromEmail_(evidenceEmail)
        || guessSingleNameFromEmailLocal_(evidenceEmail)
        || guessContactNameFromEmail_(evidenceEmail)
        || ""
      );
      const repairedRoleLabel = buildRoleBasedContactFallbackLabel_(
        evidenceEmail,
        evidenceCompany || rec.empresa || "",
        sourceUrl
      );
      const repairedPersonAllowed =
        !!repairedPersonName
        && (
          isNameConsistentWithEmailLocal_(repairedPersonName, evidenceEmail)
          || isSingleTokenNameAllowedByEmail_(repairedPersonName, evidenceEmail, evidenceCompany || rec.empresa || "", sourceUrl)
          || looksLikePersonName_(repairedPersonName, evidenceCompany || rec.empresa || "", sourceUrl)
        );
      if (repairedPersonAllowed) {
        evidenceName = repairedPersonName;
        addDiagnostic(`Ajustado "${recLabel}": nombre reparado por coherencia con email (${evidenceName}).`);
      } else if (repairedRoleLabel) {
        evidenceName = repairedRoleLabel;
        addDiagnostic(`Ajustado "${recLabel}": sin nombre personal fiable; se usa etiqueta de contacto (${evidenceName}).`);
      } else {
        droppedUnverified += 1;
        droppedMissingName += 1;
    ...
    const verifiedEmailRoot = evidenceEmailKey
      ? getRootDomain_(String(evidenceEmailKey.split("@")[1] || "").trim())
      : "";
    const verifiedDomainRoot = verifiedEmailRoot || sourceRootDomain || "";
    if (
      verifiedDomainRoot
      && Number(verifiedDomainJobCount[verifiedDomainRoot] || 0) >= verifyPerDomainCapPerJob
    ) {
      droppedUnverified += 1;
      droppedDomainCap += 1;
      addDiagnostic(`Descartado "${recLabel}": tope por dominio en ejecucion (${verifiedDomainRoot}, max=${verifyPerDomainCapPerJob}).`);
      continue;
    }
    if (
      verifiedDomainRoot
      && Number(verifiedDomainStepCount[verifiedDomainRoot] || 0) >= verifyPerDomainCapPerStep
    ) {
      droppedUnverified += 1;
      droppedDomainCap += 1;
      addDiagnostic(`Descartado "${recLabel}": tope por dominio en paso (${verifiedDomainRoot}, max=${verifyPerDomainCapPerStep}).`);
      continue;
    }
    ...
    if (verifiedDomainRoot) {
      verifiedDomainStepCount[verifiedDomainRoot] = Number(verifiedDomainStepCount[verifiedDomainRoot] || 0) + 1;
      verifiedDomainJobCount[verifiedDomainRoot] = Number(verifiedDomainJobCount[verifiedDomainRoot] || 0) + 1;
    }
    addDiagnostic(
  ...
  return {
    ...
    droppedDomainCap: droppedDomainCap,
...
function normalizePersonName_(rawName) {
  ...
  for (let i = 0; i < tokens.length; i += 1) {
    ...
    if (/(oficina|office|webmaster|adminweb|bandas|pizzapop|call|center|contacto|contact|info|support|soporte|noticias|noticia|news|new|latest|update|updates|paper|papers|volumen|brutal|lanzamientos|conciertos|publicidad|marketing|comercial|redaccion|comunicacion|redes|social|sociales|facebook|instagram|twitter|tiktok|youtube|linkedin|siguenos|follow|whatsapp|quienes|somos|http|https)/i.test(token)) {
      return "";
    }
  }
...
function hasCompanyLikeWords_(lowerText) {
  ...
  if (!value) return false;
  return /(s\.?l\.?|s\.?a\.?|ltda|sl|sa|records|magazine|radio|media|agency|agencia|producciones|production|festival|shop|store|events|management|promociones|group|grupo|fundacion|fundación)/i.test(value);
}
...
function guessSingleNameFromEmailLocal_(email) {
  ...
  if (!localRaw) return "";
  const localAlphaHint = localRaw.replace(/[0-9]+/g, "");
  if (!/[a-z]/i.test(localAlphaHint)) return "";
...
function extractLikelyPersonNamesFromText_(text, maxItems) {
  ...
  const out = [];
  const seen = {};
  const regex = /([A-ZÁÉÍÓÚÑÜ][a-záéíóúñü'`.-]{1,24}(?:\s+[A-ZÁÉÍÓÚÑÜ][a-záéíóúñü'`.-]{1,24}){0,3})/g;
  let match;
...
function dedupeRecords_(records, seenHashes, seenEmails, seenCompanies) {
  const hashArray = Array.isArray(seenHashes) ? seenHashes : [];
  const emailArray = Array.isArray(seenEmails) ? seenEmails : [];
  const companyArray = Array.isArray(seenCompanies) ? seenCompanies : [];
  const seenMap = {};
  const seenEmailMap = {};
  const seenCompanyMap = {};
  ...
  for (let i = 0; i < companyArray.length; i += 1) {
    const company = normalizeCompanyName_(String(companyArray[i] || "").trim());
    const key = normalizeTextForMatch_(company);
    if (!company || !key) continue;
    seenCompanyMap[key] = true;
  }
  ...
  for (let i = 0; i < records.length; i += 1) {
    ...
    const company = normalizeCompanyName_(String(r.empresa || "").trim());
    const companyKey = normalizeTextForMatch_(company);
    if (company && companyKey && !seenCompanyMap[companyKey]) {
      seenCompanyMap[companyKey] = true;
...
function collectExistingDedupState_(sheet, columns, scope) {
  ...
  if (!sheet || !Array.isArray(columns) || !columns.length) {
    return { seenHashes: outHashes, seenEmails: outEmails, seenCompanies: outCompanies };
  }
...
function scoreEmailDomainFit_(email, rec, sourceUrl, pages, preferredCandidate, allowRoleMailboxEmails) {
  ...
  const recCompany = normalizeCompanyName_(String(recObj.empresa || recObj.company || "").trim());
  const recName = normalizePersonName_(
    String(recObj.nombre_contacto || recObj.contacto || recObj.nombre || recObj.name || "").trim()
  );
  const sourceRoot = getRootDomain_(String(sourceUrl || ""));
  const list = Array.isArray(pages) ? pages : [];
  const pageRoots = {};
  ...
  if (recName) {
    if (isNameConsistentWithEmailLocal_(recName, value)) {
      score += 58;
    } else if (isSingleTokenNameAllowedByEmail_(recName, value, recCompany, recRoot || sourceRoot || root || "")) {
      score += 24;
    } else if (!isGenericMailboxEmail_(value) && !isRoleLikeMailboxEmailStrict_(value)) {
      score -= 32;
    }
  }
...
function saveSearchJobState_(state) {
  ...
  if (raw.length > 75000) {
    ...
    state.seenCompanies = (state.seenCompanies || []).slice(-900);
...
function compactJobStateForStorage_(state) {
  ...
  if (!Array.isArray(state.seenCompanies)) state.seenCompanies = [];
  ...
  if (state.seenCompanies.length > 12000) {
    state.seenCompanies = state.seenCompanies.slice(-12000);
  }
...
function saveJobStateToPersistentStore_(state, rawState, force) {
  ...
  try {
    ...
    if (raw.length > 380000) {
      ...
      state.seenCompanies = (state.seenCompanies || []).slice(-8000);
      raw = JSON.stringify(state);
    }
